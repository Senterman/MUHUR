 ...
 * EGL: eglSwapInterval(0) + basit config — çalışan temel üzerine kurulu
 */
...
enum State{ ST_CINEMA, ST_MENU, ST_SETTINGS, ST_BOOT };
static State g_state = ST_CINEMA;

static int   g_scene     = 0;
static float g_typePos   = 0.f;
static bool  g_textDone  = false;
static long  g_sceneStart= 0;
static bool  g_skipHeld  = false;
static long  g_skipStart = 0;
static float g_skipBar   = 0.f;
...
/* ════ RENDER SİNEMATİK ════ */
static void updateTW(){
    const char* txt=SC_TEXT[g_scene<6?g_scene:5];
    ...
    }else if(g_skipBar>0)g_skipBar*=0.85f;
}
...
static void renderCinema(){
    ...
    int si=g_scene<6?g_scene:5;
    if(g_scTex[si]) Tex(g_scTex[si],0,0,sw,sh,1.f);
    else{float fb[4]={0.3f,0.2f,0.02f,1.f};Rect(0,0,sw,sh,fb);}
    ...
    float skc[4]={GDIM[0],GDIM[1],GDIM[2],1.f};
    if(g_skipBar>0){skc[0]=GOLD[0];skc[1]=GOLD[1];skc[2]=GOLD[2];}
    float skps=sw*.007f;
    float sktw=(float)nCh("SKIP")*7*skps;
    drawStr("SKIP",skx+(skw-sktw)*.5f,sky+(skh-9*skps)*.5f,skps,skc);
    if(g_skipBar>0.01f) Rect(skx,sky+skh-sh*.003f,skw*g_skipBar,sh*.003f,GOLD);

    /* DEVAM butonu — metin bitince */
    if(g_textDone){
        ...
        float dps=sw*.007f;
        float dtw=(float)nCh("DEVAM")*7*dps;
        drawStr("DEVAM",nx+(nw-dtw)*.5f,ny+(nh-9*dps)*.5f,dps,GOLD);
    }
}
...
static void renderSettings(){
    ...
    scanLines();
    Rect(0,sh*.010f,sw,sh*.004f,GOLD);
    drawStrC("AYARLAR",sh*.055f,sw*.011f,GOLD);
...
static void onDown(float tx,float ty){
    if(g_state==ST_CINEMA){
        ...
        float skw=sw*.20f,skh=sh*.055f;
        float skx=sw-skw-sw*.03f,sky=sh*.015f;
        if(hitR(skx,sky,skw,skh,tx,ty)){g_skipHeld=true;g_skipStart=nowMs();}
    }
    if(g_state==ST_MENU)
...
static void onUp(float tx,float ty,android_app* aapp){
    ...
    if(g_state==ST_CINEMA){
        ...
        if(g_textDone){
            float nw=sw*.30f,nh=sh*.058f;
            float nx=sw-nw-sw*.04f,ny=sh*.905f;
            bool devamHit=hitR(nx,ny,nw,nh,tx,ty);
            if(devamHit||true){ /* Ekrana her yere tıklayınca ilerle */
                g_scene++;
...
static void onCmd(android_app* aapp,int32_t cmd){
    ...
    case APP_CMD_INIT_WINDOW:{
        ...
        const EGLint att[]={
            ...
            EGL_GREEN_SIZE,8,
            EGL_BLUE_SIZE, 8,
            EGL_NONE  /* alpha yok — opaque */
        };
        EGLConfig cfg; EGLint nc=0;
        if(!eglChooseConfig(g_dpy,att,&cfg,1,&nc)||nc==0){
            const EGLint att2[]={EGL_SURFACE_TYPE,EGL_WINDOW_BIT,
                EGL_RENDERABLE_TYPE,EGL_OPENGL_ES2_BIT,EGL_NONE};
            if(!eglChooseConfig(g_dpy,att2,&cfg,1,&nc)||nc==0){LE("cfg");break;}
        }
        ...
        for(int i=0;i<6;i++) g_scTex[i]=loadTex(SC_ASSET[i]);
...
void android_main(android_app* aapp){
    ...
    while(!aapp->destroyRequested){
        ...
        switch(g_state){
            case ST_CINEMA:   updateTW(); renderCinema(); break;
